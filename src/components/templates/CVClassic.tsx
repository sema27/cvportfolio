...
export function CVClassic({ data }: { data: CVData }) {
  const { personal, experience, education, skills } = data

  return (
    <div className="min-h-full flex flex-col font-sans text-slate-800">
      {/* Classic header: centered, simple */}
      <header className="border-b-2 border-slate-800 px-8 py-6 text-center">
        <h1 className="text-2xl font-display font-bold text-slate-900 tracking-tight">
          {personal.fullName || 'Ad Soyad'}
        </h1>
        {personal.title && (
          <p className="text-slate-600 mt-1">{personal.title}</p>
        )}
        <div className="flex flex-wrap justify-center gap-x-4 gap-y-1 mt-2 text-sm text-slate-600">
          ...
          {personal.linkedin && (
            <>
              <span>|</span>
              <a href={personal.linkedin} className="text-slate-800 underline" target="_blank" rel="noopener noreferrer">LinkedIn</a>
            </>
          )}
          {personal.website && (
            <>
              <span>|</span>
              <a href={personal.website} className="text-slate-800 underline" target="_blank" rel="noopener noreferrer">Web</a>
            </>
          )}
        </div>
      </header>
      ...
      <div className="flex-1 px-8 py-6 space-y-5">
        {personal.summary && (
          <section>
            <h2 className="text-sm font-display font-bold text-slate-900 border-b border-slate-300 pb-1 mb-2">
              Özet
            </h2>
